(function (LG) {
  ...
  function statCard(id, label, data) {
    ...
    [["-", -1], ["+", 1]].forEach(([text, amount]) => {
      ...
      button.type = "button";
      button.disabled = busy || (amount < 0 && value + (draft[id] || 0) <= 0);
      button.addEventListener("click", () => {
        draft[id] = (draft[id] || 0) + amount;
        renderStats();
      });
  ...
  function renderStats() {
    ...
    summary.append(node("div", "", `可用属性点 ${points}`),
    ...
    Object.entries(LG.CAREER_DATA.stats).forEach(([id, label]) =>
      grid.append(statCard(id, label, data)));
  ...
  function factionCard(item) {
    ...
    const button = node("button", "", item.joined ? "本轮选择此势力" : "递交聘用证明");
    button.type = "button";
    button.disabled = busy || Boolean(LG.career.data().joinedThisRun);
