(function (LG) {
  ...
  function card(item, state) {
    ...
    const useButton = node("button", "quiet-button", "食用");
    useButton.type = "button";
    const adult = LG.blackMarket.currentAge(state) >= 18;
    useButton.disabled = busy || !item.quantity || !adult;
    useButton.textContent = !adult ? "18岁后食用" : item.quantity ? "食用" : "无库存";
