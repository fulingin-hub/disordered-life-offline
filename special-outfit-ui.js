(function (LG) {
  ...
  function effectText(id, ordinary) {
    ...
    if (ordinary?.saintSet) {
      return "自我意志不灭：锁定坏结局；异界联盟魔境失败不增加败北值、不能满足欲望，胜利额外获得500人格值，完整一轮再获得500属性点与500人格值";
    }
    if (ordinary?.edenSet) {
      return "伊甸园的馈赠：完成任务时额外获得一次同额报酬";
    }
    if (ordinary?.penitentiarySet) {
      return "影狱权限：灰色地带与影狱角色商店的饮品、食物、药剂全部免费";
    }
    if (ordinary?.realmHunterSet) {
      return "异界联盟猎人：跳过七层地狱小怪；每层按层数获得人格值平衡+600；完成欲望任务额外获得败北值平衡+100";
    }
    if (ordinary?.realmBlackKnightSet) {
      return "魔境黑骑：异界联盟胜利额外获得200人格值；完整一轮额外获得5000属性点与5000人格值；完成欲望任务额外获得100败北值";
    }
